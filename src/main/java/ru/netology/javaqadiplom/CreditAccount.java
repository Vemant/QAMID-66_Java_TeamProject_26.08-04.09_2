...
public class CreditAccount extends Account {
    ...
    public CreditAccount(int initialBalance, int creditLimit, int rate) {
        if (initialBalance <= -creditLimit) {
            throw new IllegalArgumentException(
                    "Начальный баланс должен быть выше кредитного лимита"
            );
        }
        if (creditLimit <= 0) {
            throw new IllegalArgumentException(
                    "Абсолютное значение кредитного лимита должно быть положительным, а у вас: " + creditLimit
            );
        }
        if (rate <= 0) {
            throw new IllegalArgumentException(
    ...
    /**
     * Операция расчёта процентов на отрицательный
     * баланс счёта при условии, что
     * счёт не будет меняться год.
     * Сумма процентов приводится к целому
     * числу через отбрасывание дробной части
     * (так и работает целочисленное деление).
     * Пример: если на счёте -200 рублей,
     * то при ставке 15% ответ должен быть -30.
     * Пример 2: если на счёте 200 рублей,
     * то при любой ставке ответ должен быть 0.
     *
     * @return
     */
    @Override
